...
The Notification Center Web Component has a few properties that you can use to customize the behaviour of the component. The only required property is `applicationIdentifier`.
...
| subscriberHash | string | The hashed subscriber id, more info https://docs.novu.co/notification-center/react/react-components/#enabling-hmac-encryption. |
| stores | object[] | The array of the store items that are bound with the feed, it's used together with tabs property if you would like to have separate feeds in the notification center. More information can be found https://docs.novu.co/notification-center/react/react-components/#multiple-tab-layout. |
| tabs | object[] | The array of items that do describe the tabs/feeds, they are bound with the storesproperty. More information can be found https://docs.novu.co/notification-center/react/react-components/#multiple-tab-layout. |
...
| theme | object | The object defining the the light/dark styles of the Notification Center component, more info https://docs.novu.co/notification-center/react/react-components#customizing-the-notification-center-theme. We discourage you to use this prop to do the styling, instead it's recommended to use the styles property. |
| styles | object | The object allows you to define the custom CSS code. Check the details https://docs.novu.co/notification-center/custom-styling. |
...
| i18n | object | The object allowing to customize the UI language, more details https://docs.novu.co/notification-center/react/react-components/#customize-the-ui-language. |
| onLoad | function | The callback function that is called when the session is successfully initiated. It receives the first argument object with the organizationproperty. There is a sibling property sessionLoaded that might be used in the environments that do not allow to use properties with the prefix on, like in Angular. |
