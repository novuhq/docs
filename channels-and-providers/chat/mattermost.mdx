...
// Store the Mattermost webhook URL in the subscriber entity.
novu.subscribers.setCredentials('subscriberId', ChatProviderIdEnum.Mattermost, {
  webhookUrl: 'https://example.com/mattermost/webhooks/incoming',
});
```


Once you have stored the webhook URL in the subscriber entity, you can send a notification to Mattermost using the following code: 

```jsx
import { Novu } from '@novu/node'; const novu = new Novu("<NOVU_API_KEY>"); 
// Send a notification to Mattermost.
novu.notifySubscriber('subscriberId', { message: 'This is a notification from your application!', });
...
const botApiToken = await novu.subscribers.getCredentials('subscriberId', ChatProviderIdEnum.Mattermost).then(credentials => credentials.botApiToken);

// Send a notification to Mattermost using the bot API token.
novu.notifySubscriber('subscriberId', {
