...
<Note>Add `REDWOOD_ENV_APPLICATION_IDENTIFIER` and `REDWOOD_ENV_SUBSCRIBER_ID` to the `.env` file. </Note>
...
<Note> `{{ name }}` and `{{ message }}` are payload values that will be passed on from our code to the workflow template so it can appear in the notification sent to subscribers.</Note>
