...
To initialize the library and configure it with your API key, you can create an instance of the Novu module in one of your Elixir modules or scripts.

```elixir

# Using the API key only
api_key = "API_KEY"
novu = Novu.new(api_key)

# Using the API key and custom configuration
api_key = "API_KEY"
config = %{backend_url: "URL"}  # Replace "URL" with your backend URL if needed
novu = Novu.new(api_key, config: config)
