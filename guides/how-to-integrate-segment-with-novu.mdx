...
```javascript
const NOVU_WORKFLOW_IDENTIFIER = '<YOUR_WORKFLOW_IDENTIFIER>';
const SUBSCRIBER_UNIQUE_IDENTIFIER = '<YOUR_SUBSCRIBER_IDENTIFIER>';
const NOVU_API_KEY = '<YOUR_NOVU_API_KEY>';
const API_ENDPOINT = 'https://api.novu.co/v1/events/trigger';

async function onTrack(event, settings) {
  const payload = {
    name: NOVU_WORKFLOW_IDENTIFIER,
    to: {
  ...
  try {
    ...
    // Handle success or other specific statuses here
    // You might want to check for different response codes and handle them accordingly
  } catch (error) {
    // Log the error for debugging
    console.error(error);

    // Retry logic can be added here if needed, with a retry count limit
    // Retry example: if (retryCount < maxRetryCount) retryCount++;
  }
}
```

<Accordion title="Create Settings and Secrets">
...
<Frame>
  <img src="/images/guides/segment-novu-integration/editor-test.gif" />
</Frame>
