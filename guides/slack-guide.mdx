...
description: 'Leverage the integration store to send Slack notifications'
icon: 'circle-4'
---
...
You can find the corresponding docs for the setup [here](https://docs.novu.co/channels-and-providers/chat/slack).
...
After having created the subscriber, it runs the trigger code for the workflow we had created. You can find the trigger workflow by clicking on the 'Get Snippet' button on the workflow:
...
After having created the Slack app and the backend, there are two ways to configure the Slack app. One is the Novu managed option and the other is the manually managed option. 
Since the Novu managed is fairly straightforward, I'll demonstrate the manual method here. Follow along to set it up!

1. Goto 'Incoming Webhooks' in your app settings and turn it on.
...
5. Now, add this code to the backend trigger code. In our case, we'll add it to the `Novu.js` file we'd created earlier so that the final file becomes:
```jsx
import { Novu, ChatProviderIdEnum } from '@novu/node';
...
```
6. Finally, using the route we created earlier, add the API endpoint to the redirect URL section, as shown:
...
For our demonstration purposes, the front end is pretty basic. All we have to do is have an input field to take the notification text in and then send the payload. You can see the code for it [here](https://github.com/novuhq/slack-app/tree/main/frontend). 
