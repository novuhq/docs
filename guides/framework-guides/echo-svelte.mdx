...
```jsx
  npm install @novu/echo svelte-email
```

### 2. Integrate Novu with SvelteKit

Within the `src/routes` directory, create an `api/echo` folder and add a `+server.ts` file to it.
...
Within the `src/routes` directory, create an `emails` folder and add an email template in a `.svelte` file to it. 
...
1. Run the Studio
<Frame caption="Novu Dev Studio on the first run"><img src="/images/echo-svelte-email/dev-studio-first-run.png" /> </Frame>

**Note:** Use the port on which your Svelte app is running for the echo endpoint so that the Novu Dev Studio can connect to your API route.

2. Check out the email workflow and test

<Frame caption="Preview Email workflow with Step Inputs & Payload"><img src="/images/echo-svelte-email/email-workflow-preview.png" /> </Frame>
