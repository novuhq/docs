...
Learn how to send notifications with Remix, React email and Novu. You can check out the complete code for a [working app](https://github.com/novuhq/novu-echo-remix-example).
...
```jsx
  npm install @novu/echo @react-email/components react-email
```

### 2. Integrate Novu with Remix

Within the `app/routes` directory, create an `api.echo.tsx` file.

```jsx
// app/routes/api.echo.tsx
...
Within the `app` directory, create an `echo` folder and add a `client.ts` file to it. Copy/paste the code below to the recently created file.
...
1. Run the Studio
<Frame caption="Novu Dev Studio on the first run"><img src="/images/echo-remix/novu-dev-studio-first-run.png" /> </Frame>

**Note:** Use the port on which your Remix app is running for the echo endpoint so that the Novu Dev Studio can connect to your API route as highlighted in the image above.

2. Check out the signup email workflow and test

<Frame caption="Preview Email workflow with Step Inputs & Payload"><img src="/images/echo-remix/signup-email-workflow-preview.png" /> </Frame>
