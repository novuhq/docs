---
title: 'How to use FCM with Novu'
...
You can find the entire code (frontend as well as backend) for this app [here](https://github.com/novuhq/).
<br/>
Push notifications are notifications that are 'pushed' to a user's devices whether the user is using the app or not. We'll be using Firebase Cloud Messaging or FCM integration of Novu to send these notifications. This guide will be broken down into three parts:
1. Setting up Novu
2. Setting up Firebase
3. Adding Firebase to the Frontend, and
...
3. Because we're creating a web app here, we'll choose the 'web' option.
...
5. We're done with the Firebase setup! Now, we'll set up Novu and finally proceed to add both to our app.

## Setting up Novu
We'll use a workflow to trigger notifications to our user's devices, which we need to create first. To ensure that FCM works as expected, follow these steps:
...
```
Here, we're creating a subscriber with the `subscriberID` of whatever value the `env` file contains for the identifier `NOVU_SUB_ID`. You can read more about subscribers [here](/subscribers/subscribers).
...
We're done with the app. You can find the complete code for the app [here.]()
