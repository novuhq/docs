...
<Note>
  ...
  look [here](/legacy-guides/add-digest-to-inapp-notifications).
</Note>
...
<Frame caption="Adding a digest node to an email notification workflow">
  <img src="/images/digest-email.png" />{" "}
</Frame>
The `digest` node allows you to set a specific time interval for when notifications
should be sent:
<Frame caption="Setting time-interval in digest node">
  <img src="/images/digest.png" />{" "}
</Frame>
Once, you’ve configured it. Go ahead and configure the `email` channel as per your
need:
<Frame caption="Configuring the email node as per our requirements">
  <img src="/images/email-node.png" />{" "}
</Frame>
...
<Frame caption="Getting trigger code is simple">
  <img src="/images/trigger-code.png" />{" "}
</Frame>
...
<Frame caption="Filtering by channel">
  <img src="/images/email-digest.gif" />{" "}
</Frame>
