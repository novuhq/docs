...
<Steps>
    ...
    <Step title="Create a Next.js application">
        This link can be copied right from the onboarding guide on the Novu Studio or can always be copied from the [API Keys](https://web.novu.co/api-keys/Development) page on the Novu Dashboard.

        ```bash
        npx create-novu-app --secret-key=<YOUR_NOVU_SECRET_KEY>
        ```
