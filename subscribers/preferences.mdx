...
<AccordionGroup>
  ...
  <Accordion title="What preferences are applied to a subscriber when we create a new workflow?">
    ...
    You can turn off all channels of workflow for a subscriber using [subscriber preference](/subscribers/preferences#subscriber-level-channel-preferences). if all channels are off, then subscriber will not receive any message from any channel step and hence workflow is disable for that subscriber
  </Accordion>
</AccordionGroup>
