...
In Novu, we call the entities designed to receive notifications "Subscribers". Each subscriber is unique and identified by a unique subscriber ID.
...
## Subscriber response schema[](https://docs.novu.co/platform/subscribers#subscriber-response-schema)
...
### 1. Ahead of Trigger[](https://docs.novu.co/platform/subscribers#1-ahead-of-trigger)
...
### 2. Inline of Trigger[](https://docs.novu.co/platform/subscribers#2-inline-of-trigger)
...
You can create subscribers in bulk *(up to 500 at once)* via the SDKs or [API](https://docs.novu.co/api-reference/subscribers/bulk-create-subscribers).
...
Any subscriber can be retrieved using a unique subscriber identifier `subscriberId`. Check the returned subscriber schema [here](https://docs.novu.co/platform/subscribers?language=php#subscriber-response-schema).
