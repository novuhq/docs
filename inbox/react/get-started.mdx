...
Novu provides the `@novu/react` a React library that helps to add a fully functioning notification center to your web application in minutes. Let's do a quick recap on how you can easily use it in your application.
...
You can pass the `localization` prop to the Inbox component to change the language of the notification center.
...
Novu provides a real-time socket API for you to consume and get updates about new notifications added to the user's feed. To use the socket connection you can use the `useSocket` hook provided by the `@novu/react` library. Let's see an example of that:

```tsx
import { NovuProvider, useSocket } from "@novu/react";
