...
- The `styles` props is replaced by an enchanced and easy to use `appearance` prop to customize the appearance of the notification components. For more information on `appearance` customization visit [here](/inbox/styling).
...
For more information on `appearance` customization visit [here](/inbox/styling).
...
The process remains the same as before. See the detailed guide [here](/inbox/production#hmac-encryption).
