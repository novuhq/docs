...
<AccordionGroup>
  ...
  <Accordion title="How to solve CORS error while using the notification center?">

  Novu's notificaion center is configured for all hostnames so no any other configurations required for CORS. Disable the root level `withCredentis` config in axios or fetch request.
