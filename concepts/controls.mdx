...
Controls are defined using [JSON Schema](/framework/recipes/json-schema) (Zod is supported as well), providing a strong run-time validation system for your workflows.
...
**Controls** - For Non-Technical Peers and Developers. Managed in the Novu Dashboard UI, defined by developers and used by non-technical peers.
<Info>
...
[Step](/framework/concepts/steps) control schema is defining the control passed during the `step` method. Those controls can be modified and persisted in the Novu Dashboard UI.
...
workflow('new-signup', async ({ step, payload }) => {
    await step.email(
      ...
      async (controls) => {
          return {
            subject: controls.subjectTitle,
            body: render(<ReactEmailContent hideBanner={controls.hideBanner} components={controls.components} />),
        }
      }, {
...
To Learn more about advanced examples, visit our [JSON Schema](/recipes/json-schema) page.
