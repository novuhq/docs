...
description: 'Here you larn all about notification workflows'
icon: 'sitemap'
---
...
A notification workflow acts as the blueprint for the notifications that will be sent. The workflow encapsulates the entire flow and rules of messages that are being sent. This is where all the different channels, filters, rules and actions are tied together under a single entity.
...
<Check>**Each Notification Workflow comprises the following components**</Check>

## Name and Identifier

Every notification workflow will have a name and an identifier that are used to uniquely identify each workflow.

## Trigger

The "Trigger" refers to an event or action that initiates the workflow. It signifies a call to the Novu API with a specified notification workflow ID, along with the necessary payload data that the notification workflow content will utilize.
...
# Step Filters and Rules

Workflow step filtering allows you to customize the flow of notifications in your workflow by specifying criteria for which notifications should be delivered. You can use this filter to ensure that only relevant notifications are sent to your subscribers, improving the efficiency and effectiveness of your communication.
...
2. Add a filter to the step. This will allow you to specify the criteria that must be met for the notification to be delivered.
    
<img
  style={{ borderRadius: '0.5rem' }}
  src="./images/Add-filter.png"
/>
    
3. If you want to add multiple filters, you will need to choose a logical condition between `AND` and `OR` (1). This will determine how the filters are applied to the notification. For example, if you choose `AND`, the notification will only be delivered if all of the filters are met. If you choose `OR`, the notification will be delivered if any of the filters are met.
4. Add a rule to the filter (2). This will specify the criteria that must be met for the notification to be delivered. You can use the `payload`, `subscriber`, or `webhook` field as the basis for your rule. For example, you might specify that the `payload` field must contain a certain keyword or that the `subscriber` must be in a specific group.

<Note>If only one rule is provided, `AND` and `OR` both conditions behaves in the same way.</Note>

<img
  style={{ borderRadius: '0.5rem' }}
  src="./images/Or-And-filters.png"
/>
...
**Example**: In this filter rule, the notification will only be delivered if the webhook response includes a field called `isOnline` that is set to `true`.

<img
  style={{ borderRadius: '0.5rem' }}
  src="./images/dark-webhook-rule-example.png"
/>
...
<img
  style={{ borderRadius: '0.5rem' }}
  src="./images/subscriber-online-filter.gif"
/>
...
<img
  style={{ borderRadius: '0.5rem' }}
  src="./images/subscriber-seenread-filter2.gif"
/>

<Note> For this filter to work with emails, the webhook url needs to be set up for the active `email` provider you're using on Novu. It's very essential for the filter to work. No extra set up is needed for `in-app` notifications. </Note>
...
<img
  style={{ borderRadius: '0.5rem' }}
  src="./images/dark-is-defined-filter.png"
/>

<Note> This filter can be used with subscriber, payload and webhook. It can be applied on all channels. </Note>
