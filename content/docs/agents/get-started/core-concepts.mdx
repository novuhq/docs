...
If a match exists, the handler receives subscriber information such as the subscriber ID, name, or email. If no match exists, the user is tracked as a platform user and your handler should account for cases where subscriber data is unavailable. Once identity is resolved later, they are upgraded to a full subscriber and future messages include their subscriber data.
