...
<Callout type="info">
 Attachments are limited to 25 MB per file. Files are only supported with string/markdown replies, not card replies. Provide each file with exactly one of `url` or `data`.
</Callout>
...
Cards are structured messages with buttons, dropdowns, links, and more, built with function calls or JSX.
...
Configure `tsconfig.json` with `"jsxImportSource": "@novu/framework"`, then return JSX from a handler or pass it to `ctx.reply()`. See [Build your first agent](/agents/custom-code-agent/build-your-first-agent) for a full JSX card example with `Card`, `CardText`, `Actions`, and `Button`.
