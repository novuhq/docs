...
Inbound messages can include file attachments when the platform supports them. Novu normalizes files into `message.attachments` with short-lived signed URLs:
...
<Callout type="info">
 Download attachment URLs promptly inside your handler. Signed links are valid for 15 minutes. Inbound attachments are limited to 25 MB per file.
</Callout>
