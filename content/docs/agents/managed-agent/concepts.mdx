...
You do not need to enable or configure built-in tools. They come with the connector.
...
| **Resolved** | The conversation has been closed. An optional summary is stored. |
| **Reopened** | A new user message after resolution reopens the conversation automatically. |
...
When the AI platform thinks through a problem in steps, Novu can surface that chain of thought in supported providers. Streaming is handled automatically, so responses appear progressively rather than all at once.
