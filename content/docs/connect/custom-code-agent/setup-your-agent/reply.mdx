...
import { Pencil, Signal, Brain } from 'lucide-react';
...
Replies are user-facing messages your agent sends back into the conversation. Use [signals](/connect/custom-code-agent/setup-your-agent/signals) when you need to update conversation state, trigger workflows, or resolve the thread without messaging the user.
...
To change a message after you send it, see [Edit sent messages](/connect/custom-code-agent/setup-your-agent/edit-sent-messages).
...
Configure `tsconfig.json` with `"jsxImportSource": "@novu/framework"`, then return JSX from a handler or pass it to `ctx.reply()`. For a full JSX card example with `Card`, `CardText`, `Actions`, and `Button`, see [Build your first agent](/connect/custom-code-agent/build-your-first-agent).

When a user clicks a button or selects a dropdown value, `onAction` fires with `actionId` and `value`. See [Handle events](/connect/custom-code-agent/setup-your-agent/handle-events#onaction).
