...
2. Select the Invite button. You can do this from the **Members**, **Invitations**, or **Requests** tab
3. Enter the member’s email address
4. (Optional) Assign a role to the new member. If no role is selected, the member will be assigned the **Viewer** role by default
...
<Callout type="info">You can can still send a new invitation after revoking the previous one.</Callout>
...
4. Accept or decline their request.
