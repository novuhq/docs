...
<Callout type="info">
  The Agent Toolkit is a code-level SDK for embedding Novu tools inside your own AI agents. For giving AI coding assistants (Claude Code, Cursor, Copilot) context about Novu, see [Agent Skills](/platform/additional-resources/skills). For connecting AI tools to Novu via the Model Context Protocol, see [MCP](/platform/additional-resources/mcp).
</Callout>
...
- **`requireHumanInput(tools, config)`** — Wraps tools with human-in-the-loop approval before execution.
- **`resumeToolExecution(toolCall, decision)`** — Resumes a deferred tool call after human review.
- **`handleWebhookEvent(event)`** — Parses an incoming Novu webhook event into a structured decision result.
...
- **`requireHumanInput(tools, config)`** — Wraps tools with human-in-the-loop approval.
- **`resumeToolExecution(toolCall, decision)`** — Resumes a deferred tool call after human review.
- **`handleWebhookEvent(event)`** — Parses webhook events into structured decision results.
...
The toolkit supports a **human-in-the-loop** pattern that lets you require human approval before executing sensitive tool calls. When a guarded tool is invoked, execution is deferred and a Novu workflow notifies a human reviewer. The agent receives a "pending" status instead of a result, and execution only resumes after the human approves, edits, or rejects the action.

### How It Works

1. Wrap tools with `requireHumanInput` to mark them as needing approval.
2. When the LLM invokes a guarded tool, the toolkit triggers a Novu workflow that notifies reviewers.
3. The agent receives a `pending-input` status and does not retry.
4. A human reviews the action via the notification (e.g., in-app inbox) and makes a decision.
5. Your application receives the decision via webhook and calls `resumeToolExecution` to complete or reject the action.

### Configuration

```typescript
import type { HumanInputConfig } from '@novu/agent-toolkit/openai';

const humanInputConfig: HumanInputConfig = {
  workflowId: 'approve-agent-action',
  subscribers: ['admin-user-1'],
  allowedDecisions: ['approve', 'edit', 'reject'],
  metadata: { source: 'customer-support-agent' },
  onBeforeTrigger: async (toolCall) => {
    console.log('Requesting approval for:', toolCall.method);
  },
  onAfterTrigger: async (toolCall, result) => {
...
| Option | Required | Description |
| --- | --- | --- |
| `workflowId` | Yes | The Novu workflow to trigger for the approval notification. |
| `subscribers` | Yes | Array of subscriber IDs who will receive the approval request. |
| `allowedDecisions` | No | Decision types the reviewer can make. Defaults to `['approve', 'reject']`. |
| `metadata` | No | Arbitrary metadata attached to the approval request. |
| `onBeforeTrigger` | No | Callback invoked before the approval workflow is triggered. |
| `onAfterTrigger` | No | Callback invoked after the approval workflow is triggered. |

### Decision Types

| Decision | Behavior |
| --- | --- |
| `approve` | The original tool call executes with its original arguments. |
| `edit` | The tool call executes with modified arguments provided by the reviewer. |
| `reject` | The tool call is not executed; a rejection message is returned to the agent. |
...
// Wrap specific tools to require human approval
const guardedTools = toolkit.requireHumanInput(
  toolkit.tools.filter((t) => t.function.name === 'trigger_workflow'),
  {
    workflowId: 'approve-agent-action',
    subscribers: ['admin-1'],
  }
...
// Replace original tools with guarded versions
const allTools = [
  ...toolkit.tools.filter((t) => t.function.name !== 'trigger_workflow'),
  ...guardedTools,
];
...
When a human makes a decision, Novu sends a webhook event. Use `handleWebhookEvent` to parse it and `resumeToolExecution` to complete the deferred action:
...
app.post('/webhooks/novu', async (req, res) => {
  const interactionResult = toolkit.handleWebhookEvent(req.body);

  if (!interactionResult) {
    return res.sendStatus(200);
  }

  const { toolCall, decision } = interactionResult;
  const result = await toolkit.resumeToolExecution(toolCall, decision);
...
- **Approval workflows** — Use human-in-the-loop to require manager approval before sending bulk notifications or sensitive communications.
...
│   └── human-in-the-loop/     # Human-in-the-loop approval system
├── package.json
├── tsconfig.json
└── tsup.config.ts
```
...
| `@novu/agent-toolkit/human-in-the-loop` | Human-in-the-loop utilities and types. |
