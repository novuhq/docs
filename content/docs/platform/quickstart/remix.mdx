...
import { BuildWorkflowStep, CreateAccountStep, CreateSubscriberStep, TriggerNotificationStep } from '@/components/quickstart/common-steps';
...
<Steps>
  ...
  <Step title="Create the Inbox component">
    ...
    Import the <Method href="/docs/platform/inbox/react/components/inbox">{`<Inbox />`}</Method> component and pass the <Tooltip content="The application identifier is a unique identifier for your application. You can find it in the Novu Dashboard under the API keys page.">`applicationIdentifier`</Tooltip> and <Tooltip content="The subscriber ID is the unique identifier for the user in your application, typically the user's id in your database.">`subscriberId`</Tooltip> to the component.
