...
<Callout type="info">
  ...
  [@novu/js](/platform/sdks/javascript) headless package with new dashboard workflows and [@novu/framework](/platform/workflow/overview) based workflows.
   ...
   - With old dashboard based workflows, use
...
In old implementation, you could set the avatar icon for a notification by toggle on the `Add an avatar` option in the workflow UI. Novu will use the avatar field of actor subscriber for avatar icon.

In new implementation, you can set the avatar icon for a notification by adding the avatar icon in the workflow UI. There are 3 options to choose from:

- Use a default avatar icon
- Use hard coded avatar icon url
- Use payload variable to dynamically set the avatar icon
...
The process remains the same as before. See the detailed guide [here](/platform/inbox/prepare-for-production#secure-your-inbox-with-hmac-encryption).
