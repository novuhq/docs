...
For situations where you don't need the default popover and bell trigger, you can use the `<Notifications />` or `<InboxContent />` components to be displayed just the list of notifications. This is useful for embedding the notification list directly into a page or a custom popover component.

Both components support the same customization props as `<Inbox />`, except for configuration options (like `applicationIdentifier`, `subscriber`, and `renderBell`).

<Tabs items={['Using <InboxContent />', 'Using <Notifications />']}>
  <Tab>
```tsx
