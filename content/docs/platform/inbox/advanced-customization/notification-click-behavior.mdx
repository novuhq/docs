...
Novu validates redirect URLs before delivery. Use absolute `http://` or `https://` URLs, paths starting with `/`, or template variables. Invalid URLs (for example, `mailto:` links or protocol-relative URLs) are removed, so the notification click does not navigate anywhere unless you handle it with `onNotificationClick`.
