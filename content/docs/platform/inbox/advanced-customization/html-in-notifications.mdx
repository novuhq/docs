---
pageTitle: 'Use HTML in Notification Content'
...
By default, Novu sanitizes the `subject` and `body` of all in-app step notifications to ensure they are safe to display. This process prevents security risks by removing suspicious HTML tags (like `<script>`) and escaping HTML entities (like `&`, `<`, `>`).
...
In your Novu workflow, navigate to the **In-App editor**. In the editor, find and enable the **Disable content sanitization** toggle.
...
Imagine your workflow's In-App message body contains the following content with HTML tags and variables:
