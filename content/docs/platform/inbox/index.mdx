...
<Callout type="warn">
  REST endpoints under `/v1/subscribers/{subscriberId}/notifications/*` and `/v1/subscribers/{subscriberId}/messages/*` for in-app (inbox) operations are deprecated. Use the [Inbox API reference](/api-reference/subscribers/retrieve-subscriber-notifications-count) endpoints under `/v2/subscribers/{subscriberId}/notifications/*` instead, or use the [`@novu/js`](/platform/sdks/javascript) SDK and [`@novu/react`](/platform/sdks/react) Inbox components, which call the v2 APIs for you.
</Callout>
