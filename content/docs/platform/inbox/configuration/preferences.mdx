...
You can control the order of preferences displayed in the UI by providing a callback function to the `preferenceFilter` prop. When you use a callback function, the order of preferences returned from your function determines their rendering order in the preferences UI.

#### Sorting behavior

The `preferenceFilter` prop supports two types of filtering:

- **Object-based filtering** (using `tags` and `criticality`): Filters workflows but does not control their order - they appear in their default sequence
- **Function-based filtering**: Allows you to both filter and sort workflows by returning them in your desired order

**Important**: Sorting only works when you provide a callback function to `preferenceFilter`. Using object-based filters with `tags` and `criticality` will filter workflows but will not change their display order.

#### Example: Custom sorting with callback function

To sort preferences, provide a function that receives the preferences data and returns the filtered and sorted array:
...
function InboxPreferences() {
  return (
    <Inbox
      applicationIdentifier="YOUR_APPLICATION_IDENTIFIER"
      subscriber="YOUR_SUBSCRIBER_ID"
      preferencesFilter={(preferences) => {
        // Filter and sort preferences
        return preferences
          .filter(({ workflow }) => 
            workflow.tags?.some(tag => ['security', 'account', 'marketing'].includes(tag))
          )
          .sort((a, b) => {
            // Custom sorting logic - prioritize by tag order
            const tagPriority = { 'security': 1, 'account': 2, 'marketing': 3 };
            const aTag = a.workflow.tags?.find(tag => tagPriority[tag]) || 'z';
            const bTag = b.workflow.tags?.find(tag => tagPriority[tag]) || 'z';
            return (tagPriority[aTag] || 999) - (tagPriority[bTag] || 999);
          });
      }}
    />
  );
...
#### Example: Sort by workflow name

You can sort preferences alphabetically by workflow name:
...
#### Example: Combine filtering and sorting

You can combine filtering with custom sorting logic:
...
function InboxPreferences() {
  return (
    <Inbox
      ...
      preferencesFilter={(preferences) => {
        return preferences
          // First filter for specific workflows
          .filter(({ workflow }) => 
            !workflow.critical && // Only non-critical workflows
            workflow.tags?.includes('user-facing')
          )
          // Then sort by priority
          .sort((a, b) => {
            const priorityOrder = ['urgent', 'important', 'general'];
            const aPriority = a.workflow.tags?.find(tag => priorityOrder.includes(tag));
            const bPriority = b.workflow.tags?.find(tag => priorityOrder.includes(tag));
