...
The [**`@novu/react`**](/platform/sdks/react) package provides a set of hooks, such as `useNotifications`, `useCounts`. You handle the layout, styling, and interactions while Novu provides the notification state, real-time updates, and actions.
...
<Callout> For more `NovuProvider` options, such as HMAC encryption, see the [full provider documentation](/platform/sdks/react/hooks/novu-provider). </Callout>
...
The hooks (`useNotifications`, `useCounts`) will automatically refetch and update your UI when these actions are performed.
...
If you need to listen for events manually (for example, to show a toast notification when a new message arrives), you can use the `novu.on()` method from the `useNovu` hook.
