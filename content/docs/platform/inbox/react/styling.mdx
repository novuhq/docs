...
Currently, we only support bold text. Other text formatting options will be available in the future. You can learn more about the Novu Framework in-app step [here](/framework/typescript/steps/inApp).
...
The Inbox component's styles by default are placed under the head tag of the page. However, when the Inbox component is rendered within a shadow DOM, it automatically detects it, ensuring style isolation at the shadow root level with no extra configuration required.
