...
The Inbox component is built to allow for multiple layers of styling. This allows you to style the Inbox to meet the specific requirements of your use case.

Depending on the level of customization you need, you can choose to style the Inbox component using one of the following approaches:

- [Appearance Prop](#appearance-prop)
  - [Variables](#variables) - Global primitives such as buttons, popovers, drop-downs and so on.
...
- [Custom Composition](/platform/inbox/react/components/overview#composition) - Compose Novu components for custom layouts.

<Callout type="info">
  You can see Styling in action on the [Inbox Playground](https://inbox.novu.co) with common themes such as Notion, Reddit, and more!
</Callout>

## Appearance prop

The `appearance` prop can be used to customize the Inbox. It has three main keys: `baseTheme`, `variables`, and `elements`.

- **Variables**: Global styling variables apply to multiple elements within the inbox.
- **Elements**: Elements are the individual UI components that make up the Inbox.
- **Base theme**: The base theme applied to the `<Inbox />` has the same keys as `appearance`. Use it to apply base themes like `dark`.

### Variables

Variables are used to define global styling properties that can be reused throughout the Inbox component. You might use variables to style multiple components at once. For example, to change the border radius of all the components at once, you can update the `borderRadius` variable, which modifies the CTA buttons, unseen counter, and so on.
...
The `elements` object allows you to define styles for these components. Each key corresponds to a component, and the value is an object containing `style properties` or you can also pass your `css classes`.

To find element selectors, you can inspect the elements that you want to customize using the browser's developer tools. Each element has a unique selector that you can use to style starting with `nv-`.

Strip the `nv-` prefix and add it to the `elements` object. For example, to style the `nv-notificationPrimaryAction__button` element, you can add the following to the `elements` object:
...
<Callout type="info">
  Any selector that appears before the 🔔 emoji can be targeted using the elements property in the appearance prop (stripping the `nv-` prefix). You can also use TS autocomplete to find the
  available elements.
</Callout>
...
To implement a dark theme, you can import our premade `dark` theme and apply it with the `baseTheme` option in `appearance`.
...
You can also use `CSS Modules` to style the Inbox componentsL

1. Create a CSS module file `(for example, styles.module.css)` with the styles that you want to apply to the Inbox components.
...
2. Import the CSS module file and pass the classes to the elements object.
...
<Callout type="info">
  Note: When rendering the custom notification component, you must parse the text and apply the bold styling accordingly.
</Callout>

Currently, the Inbox component only supports bold text. Other text formatting options will be available in the future. For more information, refer to the [Novu Framework in-app step documentation](/framework/typescript/steps/inApp).
