...
By default, Novu shows the subscriber preferences cog icon on the Inbox component.

Users can enable or disable any active channel in the workflow using subscriber preferences, or they can update the preference globally for all workflows under the `Global Preferences`.
...
AVOID STACKED HEADINGS, ADD PARAGRAPH HERE

### Filter preferences

You can filter the preferences visible by the user by specifying the `preferenceFilter` on the `Inbox` component. For more information, refer to [Filter visible preferences](/platform/inbox/react/components/inbox#filter-visible-preferences).
...
There are several ways to group preferences.
