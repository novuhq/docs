...
### Render based on notification severity

You can access the severity field on each notification object. This enables conditional rendering or filtering in custom UIs.
...
export default function CutomInbox() {
  ...
  return (
    <Inbox
      ...
      subscriber="YOUR_SUBSCRIBER_ID"
      renderNotification={(notification) => {
      // filter based on severity value
