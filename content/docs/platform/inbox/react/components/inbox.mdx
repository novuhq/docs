...
<Callout type="info">
  Customize the notification item using the `renderNotification` prop. The `notification.data` property allows accessing custom information while rendering the notification item.
  You can learn how to pass it with the Novu Framework in-app step output [here](/docs/framework/typescript/steps/inApp).
</Callout>
...
<Callout type="info">
  Customize visible preferences using the `preferencesFilter` prop to display only relevant preferences to your users.
  The filtering works by matching workflow [tags](/docs/workflow/overview#tags) with the specified filter `tags` value.
</Callout>
