...
<Callout type="info">
  Customize the notification item using the `renderNotification` prop. The `notification.data`
  property allows accessing custom information while rendering the notification item. You can learn
  how to pass it with the Novu Framework in-app step output
  [here](/framework/typescript/steps/inApp).
</Callout>
...
<Callout type="info">
  ...
  [tags](/workflow/overview#tags) with the specified filter `tags` value.
</Callout>
