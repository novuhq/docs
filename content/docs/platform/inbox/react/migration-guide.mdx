...
description: 'This guide outlines the key differences between the `@novu/notification-center` package and the new `@novu/react` package and provides steps to migrate your application to the latest version.'
icon: 'RefreshCcwDot'
---
...
The `@novu/react` package introduces a more flexible and customizable way to display notifications in your application. This guide:
- Outlines the key differences between the `@novu/notification-center` package and the new `@novu/react` package.
- Provides steps to migrate your application to the latest Inbox version.
...
The `styles` props is replaced by an enchanced and easy to use `appearance` prop to customize the appearance of the notification components. For more information on `appearance` customization, refer to [Styling the Inbox component](/platform/inbox/react/styling).

### Notifications

- Removal of `seen` , `lastSeenDate`, `content`, `templateIdentifier`, `payload`, `cta` properties from the Notification object.
- Introduced `archive` functionality to the Notification object.
...
After defining the feeds on the workflow UI, you can filter notifications based on the feedIdentifier.
...
The process remains the same as before. For more information, refer to [HMAC encryption](/platform/inbox/react/production#hmac-encryption).
