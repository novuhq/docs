...
Each notification can include a `data` payload that contains custom metadata such as priority, category, or activity type. You can use these data attributes to group notifications under a single tab by filtering based on specific values in the payload.
...
The data `object` supports up to two levels of nesting and must include only scalar values such as strings, numbers, booleans, or null. String values are limited to 256 characters.
