...
Each tab is defined using the `tabs` prop of the <Method href="/platform/inbox/react/components/inbox">{`<Inbox />`}</Method> component. Every tab object includes the following properties:
...
Each notification is associated with a workflow, and you can add tags to workflows in the workflow editor. You can then use these tags to group notifications from one or more workflows under a single tab by referencing shared tag names.

To use tags effectively in tab filters, assign clear and descriptive tags to your workflows in the workflow editor. For example, use a tag like `promotions` to group marketing messages or `security` to collect alerts related to user safety.
...
## Combine tags and data

You can combine both tags and data in a single filter to create specific tabs tailored to your use case.
