...
The data `object` supports up to two levels of nesting and must include only scalar values: strings, numbers, booleans, or null. String values are limited to 256 characters.
...
<Callout type="info">
  Currently, one level of nesting like `{ data: { type: 'login' } }` is supported for the `data` object filter. Deep nesting like `{ data: { details: { type: 'login' } }}` is not supported.
</Callout>
