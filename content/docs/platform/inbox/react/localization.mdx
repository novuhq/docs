...
You can use the `localization` prop to change the copywriting of the Inbox to a different language for your users or change the wording to suit your product. Refer to the list of [available keys](https://github.com/novuhq/novu/blob/next/packages/js/src/ui/config/defaultLocalization.ts#L1), or use the fully-typed TS autocomplete to find the key that you need.
