...
- <Method href="/platform/inbox/advanced-customization/customize-notification-ui#display-the-standalone-notification-feed">{`<InboxContent />`}{" "}</Method>: Renders the content of the `<Inbox />` popover.
- <Method href="/platform/inbox/configuration/preferences#using-the-preferences--component">{`<Preferences />`}{" "}</Method>: Manages subscriber notification preferences.
- `<Bell />` – Shows an unread counter and opens the inbox.
- <Method href="/platform/inbox/advanced-customization/customize-notification-ui#display-the-standalone-notification-feed">{`<Notifications />`}{" "}</Method>: Display the list of notifications.
