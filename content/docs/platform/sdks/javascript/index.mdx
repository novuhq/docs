...
<TypeTable name="NotificationFilter" type={{
  ...
  "severity": {
    "description": "Filter notifications by their severity level.",
...
<TypeTable name="PreferenceResponse" type={{
  ...
  "severity": {
    "description": "Filter preferences by their workflow's severity level.",
...
<Callout type="warning" title="Breaking change: Session Initialization and Counts">
The `session.initialize.resolved` event provides new, structured data for unread counts.
- The `totalUnreadCount` field on the session object is now deprecated.
- Please migrate to the new `unreadCount` object, which contains both the `total` count and a detailed breakdown by `severity`.
</Callout>
...
novu.on('session.initialize.resolved', ({ data }: { data: Session }) => {
  ...
  console.log(data.unreadCount.severity.HIGH);
});
