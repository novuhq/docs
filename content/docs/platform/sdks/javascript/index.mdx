...
Updates multiple workflow's channel preferences for a subscriber at once. This subscriber is identified by the `subscriberId` or `subscriber` property passed to the Novu constructor while [initializing the client](/platform/sdks/javascript#constructor-options).
...
<TypeTable name="Schedule" type={{
  ...
  "update": {
    "description": "",
    "type": "(args: UpdateScheduleArgs) => Result<Schedule>"
  }
}} />
