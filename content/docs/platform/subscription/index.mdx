...
If you use the <Method href="/platform/inbox">{`<Inbox />`}</Method>, then subscriptions determine which notifications appear in a subscriber's feed. When a subscriber joins a topic, the <Method href="/platform/inbox">{`<Inbox />`}</Method> shows notifications from that topic according to the subscriber's selected conditions. If the subscriber mutes the topic or sets rules that don't match an event, then the Inbox doesn't show those messages to that subscriber.

Subscriptions work with or without the <Method href="/platform/inbox">{`<Inbox />`}</Method>, and have a standalone <Method href="/platform/subscription/quickstart">{`<Subscription />`}</Method> component.