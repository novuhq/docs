...
Subscriptions work with or without the <Method href="/platform/inbox/react/components/inbox">{`<Inbox />`}</Method>, and have a standalone <Method href="/platform/subscription/quickstart">{`<Subscription />`}</Method> component.