...
<Callout type="info"> For the full list hooks, props, and methods, refer to the [@novu/react SDK documentation](/sdks/react). Not using React? Check out the [@novu/js SDK documentation](/sdks/javascript). For a complete reference on all available Subscription endpoints, refer to the [API reference documetation](/api-reference/overview).</Callout>
