...
If one event was collected, the rendered message would be:
...
For example, if the digest collected five events, the output would be:
...
Internally, `sentenceSummary` uses the events array to generate the sentence. To use `sentenceSummary` in the editor, select it from the variables dropdown. This will automatically insert the variable like this:
