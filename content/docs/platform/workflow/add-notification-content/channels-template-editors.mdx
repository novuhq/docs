...
title: 'Channel Template Editors'
...
The Channel template editors is where you define the content of the notification that will be delivered to your subscribers when a workflow is executed.
...
All editors (including the Subject fields) support variables and dynamic data which can be used to [personalize notification content](/platform/workflow/add-notification-content/personalize-content). Notification preview is also supported on all editors.
...
The In-App template editor defines how notifications appear inside your application through the Novu <Method href="/platform/inbox/overview">{`<Inbox />`}</Method>. In-app notifications are UI-driven, not just message driven. This means the template is responsible for both content and interaction.
...
You may disable content sanitization only when:
...
You can control how the notification avatar is displayed, you can either:
...
<Accordions type="multiple">
  <Accordion title="Create an email layout">
    ...
    3. Fill in:
      ...
      - **Identifier**: Provide a unique ID, this will be generated automatically if not provided. The identifier cannot be changed after creation.
