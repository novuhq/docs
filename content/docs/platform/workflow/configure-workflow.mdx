...
Severity levels affect how notifications appear in the Novu <Method href="/platform/inbox">{`<Inbox />`}</Method>. By default, severity controls visual cues such as color, icons, and the bell indicator, helping users prioritize messages at a glance:
