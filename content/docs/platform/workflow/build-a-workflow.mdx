...
You can define the expected payload in two ways, manually or by importing a sample. When adding or editing a schema property, you can also mark it as required.

<Callout>
  If a property is marked as required, it must be included in the payload when triggering the workflow using `novu.trigger()`.
</Callout>
...
If you already have a sample payload, you can quickly define the schema by importing it as a JSON object. Novu will infer property names, types, and structures for you.

#### Inline variable creation

While editing a workflow step content, if you reference a variable (for example, `payload.title`) that doesn’t exist in the schema, Novu will prompt you to create it inline. Clicking insert, adds the variable to your schema with a default type of `string`. You can then refine its type, mark it as required, and add validation rules.

This inline flow helps you stay in context and gradually build your schema without switching tabs or losing progress.

### Enforce schema validation

You can optionally enforce schema validation for any workflow. When this is enabled, Novu will validate incoming payloads against your defined schema when the workflow is triggered.

Schema validation can be enabled per workflow. When active, Novu validates incoming payloads against the schema when the workflow is triggered.
...
- Enum: You’ll be able to add choices, a list of predefined, allowed values. This restricts the field to only those values.
- Array: You’ll be able to select the Array item type, this defines the data type of each array element.
- Object: You’ll be able to add nested properties, each with their own type, required status, and validation options.
