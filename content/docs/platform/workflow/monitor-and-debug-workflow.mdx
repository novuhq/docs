...
From the Activity Feed, you can inspect each workflow runs and API requests to see how each subscriber notification was processed.
...
Each workflow run also includes execution details that shows how Novu processed each step for a single subscriber. The timeline is ordered chronologically and reflects the internal state transitions of each step as the workflow runs.
...
The Requests tab in the Activity Feed shows detail information about every request made to the Event API. Selecting a request opens a detailed view of how Novu processed it, the API response, and execution traces.
