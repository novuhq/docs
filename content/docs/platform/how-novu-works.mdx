...
Novu is a notification infrastructure built around the concept of workflows, environments, and subscriber-based delivery. It centralizes multi-channel messaging and supports real-time in-app experiences through a customizable <Method href="/platform/inbox">{`<Inbox />`}</Method> component.
...
The <Method href="/platform/inbox">{`<Inbox />`}</Method> component is Novu’s in-app delivery endpoint. This component makes it possible for users to receive and manage notifications directly within your application interface.
...
In-app messages are delivered to the <Method href="/platform/inbox">{`<Inbox />`}</Method> component, which is located in your frontend and listens for messages scoped to the current environment and subscriber.
...
Your application calls Novu’s API and passes in:
 ...
 - An optional `payload` to customize message content for example, `{{firstName}}`, `{{orderId}}`
  ...
  - An In-App step sends the message to the <Method href="/platform/inbox">{`<Inbox />`}</Method> component in your frontend.
