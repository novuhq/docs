...
- Each integration has:
  ...
  - An optional name, which is useful when you have multiple integrations from the same provider. Novu uses the provider ID as the default name. it is recommended to change the default name if you have multiple integrations from the same provider.
...
<Callout>If no active integration is available for a sms or email channel, Novu cannot send notifications for email and sms channels. The corresponding workflow channel step will fail.</Callout>
