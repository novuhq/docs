...
Each integration connects Novu to a specific provider, which workflows use to deliver notifications over the selected channel.
...
- Mark an integration as inactive, which means Novu will no longer use it to send messages until it is marked active again or replaced.
