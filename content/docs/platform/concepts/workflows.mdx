...
- Chat (For example, Slack, or Microsoft Teams)
...
Each channel step in a workflow includes a template editor. This is where you can configure what the user sees or receives, whether that’s an email, an SMS message, or an in-app message.
...
- Apply Liquid.js filters to format and transform values
- Use raw Liquid.js syntax for advanced logic and dynamic content

For a full explanation of how to use the template editor, personalize messages, and work with Liquid syntax, refer to the [Template Editor documentation](/platform/workflow/template-editor).
...
1. **Subscriber mapping and validation**: Subscribers are identified and validated to ensure they’re eligible to receive the notification.
2. **Workflow validation**: The system checks that the associated workflow is active and valid.
3. **Attachment upload** (*if applicable*): Any included attachments are uploaded to storage.
4. **Event queuing**: The validated event is added to a queue for efficient processing.
...
- Execution status (for example, completed, failed).
...
<Accordions type="multiple">
  ...
  <Accordion title="Can workflows be created and managed via code?">
    Workflows can be managed via code, if you are using Node.js, you can use the <a href="/framework/quickstart/nextjs">Novu Framework</a>. However, creating workflows via code is not currently supported. Please contact <a href="mailto:support@novu.co">support@novu.co</a> to get on the waitlist.
  </Accordion>
