...
- The workflow run (template used)
- The event payload
- Jobs executed
- Messages generated
...
Each step in the workflow results in one or more messages. These are the actual pieces of content sent to users emails, push notifications, SMS, in-app alerts.
...
| `jobs`          | A list of steps (jobs) that were executed as part of the workflow. Each job corresponds to a workflow action (e.g., send email, run delay). |
