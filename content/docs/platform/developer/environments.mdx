...
- **Application Identifier**
   ...
   - Used with <Method href="/platform/inbox">{`<Inbox />`}</Method>
...
When you call `GET /v1/environments` with an API secret key, the response includes decrypted API keys only for the environment that issued the key. Other environments in the response return an empty `apiKeys` array.

To return API keys for every environment in the organization, contact Novu support to enable the `IS_LIST_ENVIRONMENTS_API_KEYS_ENABLED` feature flag for your organization.

Dashboard session authentication is unchanged: users with environment read access still receive API keys for all environments they can access.
