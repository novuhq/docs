...
<Callout>This webhook feature is only available in the <a href="https://novu.co/pricing" target="_blank" rel="noopener noreferrer">Team and Enterprise plans</a>.</Callout>
...
To recover all failed messages to a particular endpoint, follow these steps:
...
4. On the menu, select a time window to recover all failed messages in the last 8 hours.
  ![Recover all failed messages menu](/images/developer-tools/recover-menu.png)
 

### Replay missing messages

You can replay all messages that were never attempted for a particular endpoint. Follow these steps:
