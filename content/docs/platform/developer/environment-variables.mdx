...
pageTitle: 'Environment variables in Novu'
...
They help you avoid hardcoding URLs, feature flags, or copy in every step, and they give you a single place to update shared configuration when something changes.

## What you can do

- Define variables with a **key** and a value per environment.
- Mark sensitive values as **secret** so they stay masked in the UI after you save them.
...
3. Create a variable: set the **key**, choose whether it is a **secret**, and enter the value for each environment that should have one.
...
Use the secret option for API keys, tokens, passwords, and other credentials. Secret values are masked in the dashboard. You cannot view the previous value after saving; you can only replace it with a new one. This reduces the risk of someone reading secrets from the UI while still letting you use them inside workflow execution.
...
Use Liquid with the `env` namespace and your variable key in uppercase or as defined in the dashboard, for example:
