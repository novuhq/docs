...
- Add your Chat provider credentials:
  ...
  - Provider-specific credentials such as API key / Auth token, Client ID, Client Secret, or
  password
...
<Step title="Update Subscriber Credentials">
  [Update](/integrations/providers/chat/overview#update-credential-webhookurl) the subscriber
  credential `webhookUrl` for this provider and integration.
</Step>

<Step title="Configure Workflow">
  Create a new [worklow](/workflow/overview) or use an existing workflow. Add chat channel and
  [write content](/content-creation-design/notification-content-creation#chat) for the message.
</Step>
