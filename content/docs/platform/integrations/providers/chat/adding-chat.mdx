...
[Update](/docs/integrations/providers/chat/overview#update-credential-webhookurl) the subscriber credential `webhookUrl` for this provider and integration.
</Step>

<Step title="Configure Workflow">
Create a new [worklow](/docs/workflow/overview) or use an existing workflow. Add chat channel and [write content](/docs/content-creation-design/notification-content-creation#chat) for the message.
</Step>
...
Checkout the [API reference](/docs/api-reference/subscribers/update-subscriber-credentials) for more details.
...
- Webhook URL for the chat channel is missing
  - `webhookUrl` field is null, undefined or not set. Check using [get subscriber api](/docs/api-reference/subscribers/get-subscriber)
