...
5. Add a description tag, then click **Create access key**. (Optionally)
6. On the **Retrieve access keys** page, you will see the **Access key** and **Secret access key** click **Download .csv file**. This file contains these access keys, you'll need in the next step. After downloading, click **Done**.
...
4. Enter a topic name, then scroll down and then click **Create topic**.
...
5. Click Subscription from the sidebar to check if the subscription was confirmed by the API
...
1. In the AWS console, search for SES (Simple Email Service), this will open the Amazon SNS dashboard.
2. In the sidebar, under **Configuration**, click **Configuration sets** 
3. Click **Create set**.
...
5.  Copy this **Configuration set name**.
...
1. Return to the Configuration Sets page in your **AWS SES dashboard** and click on the set you just created.
...
4.  For the destination, select **Amazon SNS topic** and enter a destination name.
5. Under **SNS topic**, choose the topic you created in **Step 3**.
...
3. Click **View Get set up page** and follow the steps in the **Get set up page** to verify your email address and sending domain so that you can start sending email through Amazon SES .
