...
<<<<<<< HEAD:content/docs/platform/integrations/email/(providers)/amazon-ses.mdx
import { Accordion, Accordions } from 'fumadocs-ui/components/accordion'
=======
import { Accordion, Accordions } from 'fumadocs-ui/components/accordion';
>>>>>>> docs-v3:content/docs/platform/integrations/providers/email/amazon-ses.mdx
...
<Accordions>
  <Accordion title="Trigger from novu is successful but the subscriber is not receiving email">
    Possible reasons: 1. You have not verified the subscriber's email address in SES (if you are in
    a sandbox environment). 2. Your daily sending limit has been reached (if you are in a sandbox
    environment). 3. You have entered the wrong aws region in the integration form.
  </Accordion>
</Accordions>
