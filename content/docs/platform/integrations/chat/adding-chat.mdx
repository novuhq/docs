...
- Add your Chat provider credentials:
    <Callout type="info">Each chat provider requires different type of credentials. There are few providers which does not require any credentials example: `Discord`</Callout>
...
[Update](/api-reference/subscribers/subscribers-v1-controller_update-subscriber-channel) the subscriber credential `webhookUrl` for this provider and integration.
</Step>
