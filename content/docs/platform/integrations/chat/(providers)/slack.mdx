...
<Callout>Novu provides a test Slack integration for new organizations. The integration is limited to 300 messages per month and stricter rate limits apply.</Callout>

### 1.1: Create a Slack app
First, you need to create a Slack app, this will provide you with the credential you need to create a Slack integration in Novu.
1. Go to the <a href="https://api.slack.com/apps" target="_blank" rel="noopener noreferrer">Slack API dashboard</a>.
...
These scopes are required for Novu to send messages, read channels and read users (for DMs and pickers). If you remove some of them, certain features (like channel or user selection) may not work.
...
5. Paste the Novu OAuth callback URL. Add the redirect URL that matches your Novu region:
    ...
    - EU region:
      ...
      ```
6. Click **Add** to save the URLs. If you use a self-hosted Novu instance, add your instance’s callback URL instead.

## Step 2: Configure Slack integration in Novu
...
5. Fill in the required fields using the credentials from your Slack app:
    - **Application Id**: Past your Slack app App ID.
    - **Client ID**: Paste your Slack App Client ID.
    - **Client Secret**: Paste your Slack App Client Secret.
    - **Redirect URL (Optional)**: Enter the URL where you want users to be redirected to after they successfully connect their workspace. If omitted, Novu closes the tab immedietaly after the OAuth flow completes.
...
## Step 3: Let users connect their Slack workspace
...
<Callout type="warn">The generated OAuth URL is valid for only <strong>5 minutes</strong>. Do not cache this URL; generate it dynamically when the user initiates the flow.</Callout>
...
2. Novu creates a Slack connection for that workspace, it is referenced by `connectionIdentifier` when creating endpoints.

## Step 4: Choose delivery destinations

After a workspace is connected, you or users decide where in Slack to send the messages, this can either be a Slack channel, to a user or an incoming webhook URLs. After the delivery location has been selected, a Slack endpoint is then ceeated for that location.
...
1. **Get the channel ID**: First, the channel where the message will be sent to must be selected, one way to do this is by using your Slack app token to list channels in your own UI.
    ...
    To learn more about using <a href="https://docs.slack.dev/apis/web-api/using-the-conversations-api" target="_blank" rel="noopener noreferrer">Slack conversations API</a> to either get public or private channels, refer Slack documentation.
...
1. **Get the Slack user ID**: You can use Slack’s users API to get user IDs.
    ...
    To learn more about using <a href="https://docs.slack.dev/reference/methods/users.list" target="_blank" rel="noopener noreferrer">Slack `users.list` method</a>, refer Slack documentation.
...
If `incoming-webhook` scope was included when configuring the OAuth and permissions scopes in [Step 1.1](/platform/integrations/chat/slack#12-configure-scopes-permissions). Slack would show its own channel picker during the OAuth flow:
