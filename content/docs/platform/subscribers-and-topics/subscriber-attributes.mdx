...
Each subscriber is represented as a structured object, followung a defined schema. The schema includes, the user attributes, the data field to store any custom attributes in key value pairs and channel credentials for push and chat channel provider's integrations.
...
<Callout type="info">To learn more about the subscriber schema, refer to the  [Subscribers API reference](/api-reference/subscribers/subscriber-schema) documentation.</Callout>
...
| `locale`       | string \| null | Language/region setting (e.g., `en-US`) used to localize notifications.                          |
| `timezone`     | string \| null | Timezone identifier (e.g., `America/New_York`) for scheduling and localized delivery.            |
...
You can use this field to store attributes like a user's role, membership level, plan type, or any other custom property you wish to reference in your notification templates.
...
The channels object in the subscriber profile is used to store credentials required for specific delivery providers. This ensures Novu can deliver notifications through channels that need platform-specific tokens or URLs.

Currently, it supports credentials for Push and Chat providers.
...
The channel credentials can only be updated and inserted via the API. To learn more, refer to the [Subscriber API Reference](/api-reference/subscribers/upsert-provider-credentials) documentation. 
