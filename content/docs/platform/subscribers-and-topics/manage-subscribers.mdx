...
- Fill in the subscriber details such as `subscriberId` (required), first name, last name, email, phone number, and locale.
...
- Click **Create subscriber** to save the subscriber profile.
...
const novu = new Novu({ secretKey: "YOUR_SECRET_KEY_HERE" });
 
await novu.subscribers.patch("subscriber-id-123", {
  firstName: "Albert",
  lastName: "Einstein",
  data: {
...
For more detailed information on request payloads and supported providers, refer to the Subscribers API Reference.
...
- A modal will appear; click **Delete subscriber** again to confirm.
