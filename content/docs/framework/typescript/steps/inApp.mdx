---
pageTitle: "Novu Framework TypeScript In-App Step"
title: 'In-App'
...
- **Properties**:
  - `url` (string, required): The URL to visit when clicking on the notification. Must be an absolute `http://` or `https://` URL, a path starting with `/`, or a template variable such as `{{payload.link}}`. `mailto:` links and protocol-relative URLs (for example, `//example.com`) are not allowed. Invalid URLs are removed from the notification.
...
- **Properties**:
  ...
  - `redirect` (object):
    - `url` (string, required): The URL to visit when clicking on the notification action button. Must be an absolute `http://` or `https://` URL, a path starting with `/`, or a template variable such as `{{payload.link}}`. `mailto:` links and protocol-relative URLs are not allowed.
    - `target` (string): The target attribute specifies where the new window or tab will be opened. Defaults to `_blank`. Supported values: `_self, _blank, _parent, _top, _unfencedTop`.
