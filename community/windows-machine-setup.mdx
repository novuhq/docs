...
run `nvm use vX.Y.Z`. Novu requires node v18 or higher(for example v18.17.0):
```shell
...
for Ubuntu which is the default distro installed by WSL, use the following commands
